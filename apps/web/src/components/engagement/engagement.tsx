...
function PreferenceCard({ preference, onSave }: { preference: NotificationPreference | null; onSave: (data: Record<string, unknown>) => Promise<void> }) {
  ...
  const types = [{ key: "discussion_reply", label: "Discussion replies" }, { key: "course_announcement", label: "Course announcements" }, { key: "live_class_scheduled", label: "Live class schedules" }, { key: "live_class_cancelled", label: "Live class cancellations" }, { key: "assignment_graded", label: "Assignment grades" }, { key: "certificate_issued", label: "Certificates" }];
