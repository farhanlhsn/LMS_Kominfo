...
import type { CalendarEvent,DiscussionReport,InAppNotification,LiveClass,NotificationPreference } from "../../lib/lms-types";
...
export function DiscussionList({ courseId, lessonId, activityId, manager = false }: { courseId: string; lessonId?: string; activityId?: string; manager?: boolean }) {
  ...
  if (manager && selectedThreadId) return <div className="space-y-4"><button className="text-sm font-semibold text-primary" onClick={() => setSelectedThreadId(null)} type="button">Back to moderation</button><DiscussionDetail threadId={selectedThreadId} /></div>;
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <section className="space-y-3">
      ...
      {!visibleThreads.length ? <EmptyState icon={MessageSquare} title="No discussions found" description="Start a thoughtful conversation or change the current filter." /> : visibleThreads.map((thread) =>
        <Link key={thread.id} href={`/learn/courses/${courseId}/discussions/${thread.id}`} onClick={manager ? (event) => { event.preventDefault(); setSelectedThreadId(thread.id); } : undefined} className="block rounded-lg border border-border bg-card p-5 shadow-subtle transition hover:border-primary/40">
...
export function CoursePhaseNavigation({ courseId, active = "overview", instructor = false }: { courseId: string; active?: "overview" | "gradebook" | "roster" | "discussion" | "live" | "calendar" | "announcements"; instructor?: boolean }) {
