...
async function main() {
  ...
  const demoCourse10 = await prisma.course.findFirst({
    where: { organizationId: organization.id },
  });
  if (demoCourse10) {
    const learners10 = await prisma.enrollment.findMany({
      where: { organizationId: organization.id, courseId: demoCourse10.id },
      take: 3,
    });
    const eventTypes = [
      "activity.started",
  ...
  const courses11 = await prisma.course.findMany({
