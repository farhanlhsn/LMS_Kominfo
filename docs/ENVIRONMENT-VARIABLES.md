...
| Variable                       | Contoh             | Wajib     | Keterangan                                                                      |
| ------------------------------ | ------------------ | --------- | ------------------------------------------------------------------------------- |
| `LMS_CORE_VERSION`             | `1.0.0`            | tidak     | Versi core untuk validasi kompatibilitas manifest marketplace. Default `1.0.0`. |
| `PLUGIN_SECRET_ENCRYPTION_KEY` | `<random-64-char>` | production | Kunci AES plugin secret; fallback ke `ENTERPRISE_SECRET_KEY` bila tidak diisi.   |
